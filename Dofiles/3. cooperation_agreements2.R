...
dev.copy(png,'../Figures/event_study_acuerdo3.png')
dev.off()
...
data.final %>% do(fit = felm(model_formula_cov, data = ., 
  ...
  scale_x_continuous(breaks = -5:3) + 
  theme(axis.title.x = element_blank()) + theme_bw()
dev.copy(png,'../Figures/event_study_wcovariates_acuerdo3.png')
dev.off()
...
event.study.acuerdo3<-stargazer(acuerdo, acuerdo_wcov,
                       title=" ", align=TRUE, type = "text",
                       keep=c("lag_5", "lag_4", "lag_3", "lag_2", "date_0", "lead_1", "lead_2", "lead_3"),
                       keep.stat=c("n","rsq"),
                       covariate.lables=c("Lag 5", "Lag 4", "Lag 3", "Lag 2", "Date 0", "Lead 1", "Lead 2", "Lead 3"),
                       out = '../Tables/event_study_table_acuerdo3.tex'
)

#aggregate effect: 
F <- "acuerdo_wcov$coefficients[5,1]+acuerdo_wcov$coefficients[6,1]+acuerdo_wcov$coefficients[7,1]acuerdo_wcov$coefficients[8,1]=0"
...
formula_saturated <- as.formula(paste("acuerdo3 ~ ", covariates_saturated,  
                                      "| inegi + year | 0 | estado"))
...
covariates_interest <- treatments_short2
...
formula_cldz <- as.formula("acuerdo3 ~ reform | inegi + year | 0 | estado")
formula_cldz_cov <- as.formula(paste("acuerdo3 ~ reform +",
                                     paste(covariates_all, collapse = " + "),
                                     "| inegi + year | 0 | estado"))

formula_cldz_ihs <- as.formula("acuerdo4 ~ reform | inegi + year | 0 | estado")
formula_cldz_ihs_cov <- as.formula(paste("acuerdo4 ~ reform +",
                                         paste(covariates_all, collapse = " + "),
                                         "| inegi + year | 0 | estado"))
...
plotdata_cov %>% 
  ...
  coord_flip() + 
  scale_colour_brewer(palette = 'Set1')
dev.copy(png,'../Figures/CDLZ_cov_acuerdo3.png')
dev.off()
...
plotdata_ihs_cov %>% 
  ...
  coord_flip() + 
  scale_colour_brewer(palette = 'Set1')
dev.copy(png,'../Figures/CDLZ_ihs_cov_acuerdo4.png')
dev.off()
...
formula_cldz <- as.formula(paste("acuerdo3 ~ ",
...
formula_cldz2 <- as.formula(paste("acuerdo3 ~ lag_5+lag_4+lag_3+lag_2+date_0+lead_1+lead_2+lead_3+",
                                  paste(covariates_all, collapse= "+"),
                                  "| factor(inegi):factor(df) + factor(year):factor(df) | 0 | estado"))
...
stacked_data %>% 
  ...
  scale_x_continuous(breaks = -5:3) + 
  theme_bw()
dev.copy(png,'../Figures/stacked_dataset_acuerdo3.png')
dev.off()
...
stacked_data %>% 
  ...
  scale_x_continuous(breaks = -5:3) + 
  theme_bw()
dev.copy(png,'../Figures/stacked_dataset_wcontrols_acuerdo3.png')
dev.off()
