...
model_formula_ihs <- as.formula(paste("ihs_defuncionespc ~", 
                                  paste(treatments_full, collapse = " + "),
                                  "| estado + year | 0 | 0 ")
)

model_formula_old <- as.formula(paste("loghomicide_oldpc ~", 
                                      paste(treatments_full, collapse = " + "),
                                      "| estado + year | 0 | 0 ")
)
...
model_formula_ihs_cov <- as.formula(paste("ihs_defuncionespc ~ lag_8 + lag_7 + lag_6 + lag_5 + lag_4 + lag_3 + lag_2 + date_0 + lead_1 + lead_2 + lead_3 +", 
                                      paste(covariates_all, collapse = " + "), 
                                      "| estado + year | 0 | 0 ")
)

model_formula_old_cov <- as.formula(paste("loghomicide_oldpc ~ lag_8 + lag_7 + lag_6 + lag_5 + lag_4 + lag_3 + lag_2 + date_0 + lead_1 + lead_2 + lead_3 +", 
                                          paste(covariates_all, collapse = " + "), 
                                          "| estado + year | 0 | 0 ")
)
...
data.wleadlags %>% do(fit = felm(model_formula_ihs, data = ., 
  ...
  scale_x_continuous(breaks = -7:3) + 
  theme(axis.title.x = element_blank()) + theme_bw()
dev.copy(png,'../Figures/event_study_ihs.png')
dev.off()
...
data.wleadlags %>% do(fit = felm(model_formula_ihs_cov, data = ., 
  ...
  scale_x_continuous(breaks = -7:3) + 
  theme(axis.title.x = element_blank()) + theme_bw()
dev.copy(png,'../Figures/event_study_ihs_wcovariates.png')
dev.off()
...
data.wleadlags %>% do(fit = felm(model_formula_incumbency, data = ., 
  ...
  theme(axis.title.x = element_blank()) + theme_bw()
dev.copy(png,'../Figures/event_study_incumbency.png')
dev.off()
...
event.study<-stargazer(homicides, homicides_wcov, homicidespc, homicidespc_wcov,  loghomicides, loghomicides_wcov,
                       ...
                       keep.stat=c("n","rsq"),
                       #covariate.lables=c("Lag 7","Lag 6", "Lag 5", "Lag 4", "Lag 3", "Lag 2", "Date 0", "Lead 1", "Lead 2", "Lead 3"),
                       out = '../Tables/event_study_transformations_table.tex'
)
...
AS_data  %>% 
  ...
  theme(axis.title.x = element_blank()) + theme_bw()
dev.copy(png,'../Figures/CATTs.png')
dev.off()
...
# first get percentage contribution to each lead/lag indicator by treatment cohort for weights
# calculate weights
treatments.incumbency <- c("lag_6","lag_5", "lag_4", "lag_3", "date_0", "lead_1", "lead_2", "lead_3")
...
CS_out.ihs <- att_gt("ihs_defuncionespc", idname="inegi", tname="year",
                     xformla = ~  governor_alignment,
                     data = data_CS,
                     first.treat.name="adopt_year", panel = T, control.group = "nottreated",
                     alp = 0.05, cband=T, bstrap=T, biters = 1000,
                     printdetails = F)
...
out.ihs$est.att %>% 
  ...
  theme(axis.title.x = element_text("Time relative to Treatment (2015)")) + 
  theme_bw() 
dev.copy(png,'../Figures/gsynth_ihs.png')
dev.off()
...
out.ihs_cov$est.att %>% 
  ...
  theme(axis.title.x = element_text("Time relative to Treatment (2015)")) + 
  theme_bw() 
dev.copy(png,'../Figures/gsynth_ihs_wcov.png')
dev.off()
...
plot(out.ihs, type = "ct", raw = "none", main = "", ylab = "log(homicides per capita)",
     xlab="Time relative to Treatment (2015)", shade.post = T)
dev.copy(png,'../Figures/gsynth_counterfactual_ihs.png')
dev.off()

plot(out.ihs_cov, type = "ct", raw = "none", main = "", ylab = "log(homicides per capita)",
     xlab="Time relative to Treatment (2015)", shade.post = T)
dev.copy(png,'../Figures/gsynth_wcov_counterfactual_ihs.png')
dev.off()
