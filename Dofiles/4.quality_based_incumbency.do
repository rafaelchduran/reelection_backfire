...
(1) control vs. first term mayors
(2) control vs. second term mayors
...
gen second_to_firsterm=0 if firstterm==1
replace second_to_firsterm=1 if secondterm==1

ttest incumbent_quality, by(second_to_firsterm) // those in their second term have better education levels (better selection)
ttest incumbent_quality, by(firstterm) // those in first term have lower education levels than term limited
ttest incumbent_quality, by(secondterm) // those in second term have better education levels than term limited (scareoff)
...
foreach i in firstterm secondterm second_to_firsterm{
...
did_multiplegt `outcome' group year reform, breps(5) controls($controls pol1 reform_pol1)  seed(5675) ///
cluster(estado) robust_dynamic dynamic(1) placebo(1) longdiff_placebo
