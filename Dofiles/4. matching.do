...
drop _merge
merge m:1 inegi using "../Data/ConstructionDatabase/weights_psmatching.dta"
drop if _merge==2
drop _merge
...
psmatch2 reform if ((weight_pw==. & reform==1) | (reform==0)), pscore(pscore1) outcome(logdefuncionespc) caliper (`=scalar(cal)')
...
est clear
areg  $outcome  $saturated i.year [fweight=weight_pw] if weight_pw!=., a(inegi) vce(cluster estado)
	estadd local depcontrols \checkmark
	estadd local munfe \checkmark
	estadd local yearfe \checkmark
	estadd local clustermun \checkmark
...
est clear
areg  $outcome  $saturated i.year [pweight=weight_pw2] if weight_pw2!=., a(inegi) vce(cluster estado)
	estadd local depcontrols \checkmark
	estadd local munfe \checkmark
	estadd local yearfe \checkmark
	estadd local clustermun \checkmark
