...
*========================================================================
*5) Chaisemartin and D'Haultfoeuille correction
************
*A) Acuerdo:
************
...
foreach i in 0 1 2{
	...
	glo est_t_`i'= "" 
			if (${pval_t_`i'}<=0.11) global est_t_`i' = "*"
...
foreach i in 0 1 2{
	...
	glo est2_t_`i'= "" 
			if (${pval2_t_`i'}<=0.11) global est2_t_`i' = "*"
...
tex t-2 &        $ ${pbeta_t_2}^{${pest_t_2}} $ &     $ ${pbeta2_t_2}^{${pest2_t_2}} $ \\
tex  & ($ ${pse_t_2}$) & ($ ${pse2_t_2} $) \\
tex t-1 &        $ ${pbeta_t_1}^{${pest_t_1}} $ &     $ ${pbeta2_t_1}^{${pest2_t_1}} $ \\
tex  & ($ ${pse_t_1}$) & ($ ${pse2_t_1} $) \\
tex Reform (t=0)&        $ ${beta_t_0}^{${est_t_0}} $ &     $ ${beta2_t_0}^{${est2_t_0}} $ \\
tex  & ($ ${se_t_0}$) & ($ ${se2_t_0} $) \\
tex t+1 &         $ ${beta_t_1}^{${est_t_1}} $ &       $ ${beta2_t_1}^{${est2_t_1}} $ \\
tex  & ($ ${se_t_1}$) & ($ ${se2_t_1} $) \\
tex t+2 &         $ ${beta_t_2}^{${est_t_2}} $ &      $ ${beta2_t_2}^{${est2_t_2}} $  \\
tex  & ($ ${se_t_2}$) & ($ ${se2_t_2} $) \\
...
*========================================================================
*TABLE WITH ROBUSTNESS: ONLY SHOW AGGREGATE EFFECTS
est clear
*1) No wild
qui xi: reghdfe  $outcome  $saturated $controls_time_acuerdo i.year, a(inegi) vce(cluster estado)
	...
	estadd local clustermun \checkmark
		...
		glo r2_nowild:  di %5.4f e(r2)
		glo N_nowild: di %11.2gc e(N)
	**estimate aggregate effect:
	sum perc if date_0_2015==1, meanonly
	local a = r(mean)
	sum perc if date_0_2016==1, meanonly
...
*2) Wild
qui wildcorrection_as_new2 $outcome
*xi: reghdfe  $outcome2  $saturated $controls_time_acuerdo i.year, a(inegi) vce(cluster estado)
	estadd local depcontrols
	estadd local munfe \checkmark
	estadd local yearfe \checkmark
	estadd local clustermun \checkmark
		...
		glo r2_wild:  di %5.4f e(r2)
		glo N_wild: di %11.2gc e(N)
	...
	**estimate aggregate effect:
	...
	lincom 	[(_b[date_0_2015]*`a')+(_b[date_0_2016]*`b')+(_b[date_0_2017]*`c') + (_b[date_0_2018]*`d') ///
		...
		+ (_b[lead_2_2015]*`h')+(_b[lead_2_2016]*`i') ///
		+ (_b[lead_3_2015]*`j')]/4
	glo aggregate_wild: di %5.4f r(estimate)
	estadd local aggregate_wild $aggregate_wild
	glo se_aggregate_wild: di %5.4f r(se)
	estadd local se_aggregate_wild $se_aggregate_wild
	test [(_b[date_0_2015]*`a')+(_b[date_0_2016]*`b')+(_b[date_0_2017]*`c') + (_b[date_0_2018]*`d') ///
		+ (_b[lead_1_2015]*`e')+(_b[lead_1_2016]*`f')+(_b[lead_1_2017]*`g') ///
		+ (_b[lead_2_2015]*`h')+(_b[lead_2_2016]*`i') ///
		+ (_b[lead_3_2015]*`j')]/4	=0
	glo p_aggregate_wild: di %5.4f r(p)
	estadd local p_aggregate_wild $p_aggregate_wild
		glo est_aggregate_wild= "" 
			if (${p_aggregate_wild}<=0.11) global est_aggregate_wild = "*"
			if (${p_aggregate_wild}<=0.05) global est_aggregate_wild = "**"
			if (${p_aggregate_wild}<=0.01) global est_aggregate_wild = "***"	
	


*3) Change reference period
xi: areg  $outcome  $saturated2 $controls_time_acuerdo i.year, a(inegi) vce(cluster estado) 
	estadd local depcontrols 
	estadd local munfe \checkmark
	estadd local yearfe \checkmark
	estadd local clustermun \checkmark
		...
		glo r2_ref:  di %5.4f e(r2)
		glo N_ref: di %11.2gc e(N)
	...
	**estimate aggregate effect:
	...
	lincom 	[ (_b[lead_1_2015]*`e')+(_b[lead_1_2016]*`f')+(_b[lead_1_2017]*`g') ///
		+ (_b[lead_2_2015]*`h')+(_b[lead_2_2016]*`i') ///
		+ (_b[lead_3_2015]*`j')]/3
	glo aggregate_ref: di %5.4f r(estimate)
	estadd local aggregate_ref $aggregate_ref
	glo se_aggregate_ref: di %5.4f r(se)
	estadd local se_aggregate_ref $se_aggregate_ref
	test [ (_b[lead_1_2015]*`e')+(_b[lead_1_2016]*`f')+(_b[lead_1_2017]*`g') ///
		+ (_b[lead_2_2015]*`h')+(_b[lead_2_2016]*`i') ///
		+ (_b[lead_3_2015]*`j')]/3=0
	glo p_aggregate_ref: di %5.4f r(p)
	estadd local p_aggregate_ref $p_aggregate_ref
		glo est_aggregate_ref= "" 
			if (${p_aggregate_ref}<=0.11) global est_aggregate_ref = "*"
			if (${p_aggregate_ref}<=0.05) global est_aggregate_ref = "**"
			if (${p_aggregate_ref}<=0.01) global est_aggregate_ref = "***"	
...
xi: reghdfe  $outcome  $trim $controls_time_acuerdo i.year, a(inegi) vce(cluster estado)
	...
	estadd local clustermun \checkmark
		...
		glo r2_trim:  di %5.4f e(r2)
		glo N_trim: di %11.2gc e(N)
	...
	**estimate aggregate effect:
	...
	lincom 	[(_b[date_0_2015]*`a')+(_b[date_0_2016]*`b')+(_b[date_0_2017]*`c') + (_b[date_0_2018]*`d') ///
		...
		+ (_b[lead_2_2015]*`h')+(_b[lead_2_2016]*`i') ///
		+ (_b[lead_3_2015]*`j')]/4
	glo aggregate_trim: di %5.4f r(estimate)
	estadd local aggregate_trim $aggregate_trim
	glo se_aggregate_trim: di %5.4f r(se)
	estadd local se_aggregate_trim $se_aggregate_trim
	test [(_b[date_0_2015]*`a')+(_b[date_0_2016]*`b')+(_b[date_0_2017]*`c') + (_b[date_0_2018]*`d') ///
		+ (_b[lead_1_2015]*`e')+(_b[lead_1_2016]*`f')+(_b[lead_1_2017]*`g') ///
		+ (_b[lead_2_2015]*`h')+(_b[lead_2_2016]*`i') ///
		+ (_b[lead_3_2015]*`j')]/4	=0
	glo p_aggregate_trim: di %5.4f r(p)
	estadd local p_aggregate_trim $p_aggregate_trim
			glo est_aggregate_trim= "" 
			if (${p_aggregate_trim}<=0.11) global est_aggregate_trim = "*"
			if (${p_aggregate_trim}<=0.05) global est_aggregate_trim = "**"
			if (${p_aggregate_trim}<=0.01) global est_aggregate_trim = "***"	
	

	
*Table
texdoc init  "../Tables/abraham_sun_estimates_DVaggregate.tex", replace force
tex \begin{table}[htbp]\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
tex \centering
...
tex \addlinespace
tex Observations       & ${N_nowild} & ${N_wild} & ${N_ref} & ${N_trim} \\
tex R-squared         & ${r2_nowild} & ${r2_wild} & ${r2_ref} & ${r2_trim} \\
...
*2) Naive OLS + Wild ????? why is this giving me another estimate.
...
subtitle(" ") legend(order(1 "99% CI" 2 "95% CI" 3 "90% CI") rows(1)) 
graph export "../Figures/average_effects.png", as(png) replace
