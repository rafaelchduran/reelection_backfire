...
/********************** OLD 
preserve
insheet using "../Data/ConstructionDatabase/MandoUnico/cngmd2015/sp_seguridad p£blica_cngmd2015_dbf/conjunto_de_datos/spejfues.csv", clear
rename ubic_geo inegi
rename conv_mup mando_unico_2015
rename aso_gfdm asociacion_2014
rename serv_pub tipo_cooperacion
rename inst_reg tipo_convenio
rename tipo_gob tipo_mando_unico
rename raz_conv razon_convenio
label variable mando_unico_2015 "mando unico 2015"
label variable asociacion_2014 "mando unico 2014"
label variable tipo_cooperacion "tipo cooperacion"
label variable tipo_convenio "tipo convenio"
label variable tipo_mando_unico "tipo mando unico"
label variable razon_convenio "razon convenio"
label variable tt_gobas "num. gobiernos asociacion gubernamental"
label variable est_goba "tipo falta datos de num. gob asociacion"

 
foreach i in mando_unico_2015 mando_unico_2014{
replace `i'=0 if `i'==2
replace `i'=. if `i'==8
replace `i'=. if `i'==9
}

collapse (mean) mando_unico_2015 mando_unico_2014 tipo_cooperacion tipo_convenio tipo_mando_unico razon_convenio, by(inegi)
replace tipo_cooperacion=1 if tipo_cooperacion>1 & tipo_cooperacion<8

restore


gen mando_unico=0
replace mando_unico=1 if nombre_estado=="AGUASCALIENTES" & year>=2010
replace mando_unico=1 if nombre_estado=="BAJA CALIFORNIA"
replace mando_unico=1 if nombre_estado=="BAJA CALIFORNIA SUR" & year>=2016
replace mando_unico=1 if nombre_estado=="CAMPECHE"
replace mando_unico=1 if nombre_estado=="CHIAPAS"
replace mando_unico=1 if nombre_estado=="CHIHUAHUA"
replace mando_unico=1 if nombre_estado=="COAHUILA"
replace mando_unico=1 if nombre_estado=="COLIMA"
replace mando_unico=1 if nombre_estado=="DURANGO"
replace mando_unico=1 if nombre_estado=="DISTRITO FEDERAL" & year>=2010
replace mando_unico=1 if nombre_estado=="GUANAJUATO"
replace mando_unico=1 if nombre_estado=="GUERRERO"
replace mando_unico=1 if nombre_estado=="HIDALGO"
replace mando_unico=1 if nombre_estado=="JALISCO"
replace mando_unico=1 if nombre_estado=="MEXICO" & year>=2016
/*
No han firmado:
Amanalco, Atizapán, Huixquilucan, Naucalpan, Nezahualcóyotl, Texcoco y Valle de Chalco
https://www.mexicoevalua.org/del-mito-del-mando-unico-a-la-realidad-las-policias-locales/
*/
replace mando_unico=1 if nombre_estado=="MICHOACAN"
replace mando_unico=1 if nombre_estado=="MORELOS" & year>=2013 
*replace mando_unico=1 if nombre_estado=="NAYARIT"
replace mando_unico=1 if nombre_estado=="NUEVO LEON"
replace mando_unico=1 if nombre_estado=="OAXACA"
replace mando_unico=1 if nombre_estado=="PUEBLA"
*replace mando_unico=1 if nombre_estado=="QUERETARO"
replace mando_unico=1 if nombre_estado=="QUINTANA ROO"
replace mando_unico=1 if nombre_estado=="SAN LUIS POTOSI"
*replace mando_unico=1 if nombre_estado=="SINALOA"
replace mando_unico=1 if nombre_estado=="SONORA"
replace mando_unico=1 if nombre_estado=="TABASCO"
replace mando_unico=1 if nombre_estado=="TAMAULIPAS"
replace mando_unico=1 if nombre_estado=="TLAXCALA"
replace mando_unico=1 if nombre_estado=="VERACRUZ" & year>=2013
replace mando_unico=1 if nombre_estado=="YUCATAN"
*replace mando_unico=1 if nombre_estado=="ZACATECAS"
