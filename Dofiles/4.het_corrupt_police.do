...
ap5_5_2_b  = ¿Cuánta confianza le inspira la (el) (AUTORIDAD)? policia preventiva municipal
ap5_5_8_b = ¿Cuánta confianza le inspira la (el) (AUTORIDAD)? ejercito
*/
*========================================================================
*Environment
clear all
...
************
cap drop inter_*
global split_variable ap5_5_1_mean
...
************
cap drop inter_*
global split_variable ap5_5_2_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
************
cap drop inter_*
global split_variable ap5_5_3_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
************
cap drop inter_*
global split_variable ap5_5_6_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
************
cap drop inter_*
global split_variable ap5_5_4_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
************
cap drop inter_*
global split_variable ap5_5_5_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
************
cap drop inter_*
global split_variable ap5_5_8_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
************
cap drop inter_*
global split_variable ap5_5_9_mean
foreach i in $split_variable{
foreach j in $saturated{
gen inter_`j'=`i'*`j'
}
...
*Table			
texdoc init  "../Tables/interaction_corrupt.tex", replace force
tex \begin{landscape}
tex \begin{table}[htbp]\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
tex \centering
tex \caption{Reform interaction with citizens' corruption evaluation of police forces}
tex \label{tab:interaction_corrupt}
tex \scalebox{0.70}{    
...
tex Corruption of Police Forces: & Traffic  & Preventive  & State Police & State Attorney Police & Federal Police & Ministerial Police & Army & Marines \\
...
texdoc init  "../Tables/interaction_corrupt_average.tex", replace force
tex \begin{landscape}
tex \begin{table}[htbp]\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
tex \centering
tex \caption{Reform interaction with citizens' corruption evaluation of police forces}
...
graph export "../Figures/corrupt_policeforces.png", as(png) replace
graph export "../Figures/corrupt_policeforces.pdf", as(pdf) replace
graph export "../Figures/corrupt_policeforces.tif", as(tif) replace
graph save "../Figures/corrupt_policeforces.gph", replace
*========================================================================
...
*Redo separately
**Municipal
coefplot (est1, rename((1) = ".") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 (est2, rename((1) = ".") msymbol(S) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 (est3, rename((1) = ".") msymbol(T) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 , ///
 horizontal scheme(s1color)  xline(0)    ///
ytitle(" ")  xtitle(" ") ///
subtitle("Municipal Forces") legend(order(1 "99% CI" 2 "95% CI" 3 "90% CI"  ///
4 "Reform & No Corrupt" 8 "Interaction (Reform & Corrupt)" 12 "Tot. Interaction") size(small) rows(2) region(col(white))) 
graph export "../Figures/corrupt_municipal.png", as(png) replace
graph export "../Figures/corrupt_municipal.pdf", as(pdf) replace
graph export "../Figures/corrupt_municipal.tif", as(tif) replace
graph save "../Figures/corrupt_municipal.gph", replace
**State
coefplot (est4, rename((1) = ".") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 (est5, rename((1) = ".") msymbol(S) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 (est6, rename((1) = ".") msymbol(T) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 , ///
 horizontal scheme(s1color)  xline(0)    ///
ytitle(" ")  xtitle(" ") ///
subtitle("State Forces") legend(order(1 "99% CI" 2 "95% CI" 3 "90% CI"  ///
4 "Reform & No Corrupt" 8 "Interaction (Reform & Corrupt)" 12 "Tot. Interaction") size(small) rows(2) region(col(white))) 
graph export "../Figures/corrupt_state.png", as(png) replace
graph export "../Figures/corrupt_state.pdf", as(pdf) replace
graph export "../Figures/corrupt_state.tif", as(tif) replace
graph save "../Figures/corrupt_state.gph", replace
**Federal
coefplot (est7, rename((1) = ".") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 (est8, rename((1) = ".") msymbol(S) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 (est9, rename((1) = ".") msymbol(T) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *4) color(black black black))) ///
 , ///
 horizontal scheme(s1color)  xline(0)    ///
ytitle(" ")  xtitle(" ") ///
subtitle("Federal Forces") legend(order(1 "99% CI" 2 "95% CI" 3 "90% CI"  ///
4 "Reform & No Corrupt" 8 "Interaction (Reform & Corrupt)" 12 "Tot. Interaction") size(small) rows(2) region(col(white))) 
graph export "../Figures/corrupt_federal.png", as(png) replace
graph export "../Figures/corrupt_federal.pdf", as(pdf) replace
graph export "../Figures/corrupt_federal.tif", as(tif) replace
graph save "../Figures/corrupt_federal.gph", replace

*combine:
grc1leg "../Figures/corrupt_municipal.gph" "../Figures/corrupt_state.gph" "../Figures/corrupt_federal.gph" , ///
scheme(s1color)  imargin(vsmall) ycommon xcommon  col(1) t1("Corruption") b1(Average Effect from t to t+3)
graph export "../Figures/corupt_byforces.png", as(png) replace
graph export "../Figures/corupt_byforces.pdf", as(pdf) replace
graph export "../Figures/corupt_byforces.tif", as(tif) replace
graph save "../Figures/corupt_byforces.gph", replace


*========================================================================
*Combine with efficiency
grc1leg "../Figures/performance_byforces.gph" "../Figures/corupt_byforces.gph" , ///
scheme(s1color)  imargin(vsmall) ycommon  col(2) l1(" ") b1(" ")
graph export "../Figures/efficiency_corruption.png", as(png) replace
graph export "../Figures/efficiency_corruption.pdf", as(pdf) replace
graph export "../Figures/efficiency_corruption.tif", as(tif) replace
...
/*coefplot (est1, rename((1) = "Municipal Forces") msize(large) mcolor(green) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est2, rename((1) = "Municipal Forces") msymbol(S) msize(large) mcolor(green) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est3, rename((1) = "Municipal Forces") msymbol(T) msize(large) mcolor(green) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
(est4, rename((1) = "State Forces") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est5, rename((1) = "State Forces") msymbol(S) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est6, rename((1) = "State Forces") msymbol(T) msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
(est7, rename((1) = "Federal Forces") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est8, rename((1) = "Federal Forces") msymbol(S) msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est9, rename((1) = "Federal Forces") msymbol(T) msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 , ///
 horizontal scheme(s1color)  xline(0)    ///
ytitle(" ")  xtitle("Term Limit Reform Average Effect" "from t to t+3") ///
subtitle(" ") legend(order(1 "99% CI" 2 "95% CI" 3 "90% CI"  ///
4 "Reform & No Corrupt" 8 "Interaction (Reform & Corrupt)" 12 "Tot. Interaction") size(small) rows(2) region(col(white))) 
graph export "../Figures/corrupt_policeforces2.png", as(png) replace
graph export "../Figures/corrupt_policeforces2.pdf", as(pdf) replace
graph export "../Figures/corrupt_policeforces2.tif", as(tif) replace
graph save "../Figures/corrupt_policeforces2.gph", replace
*/
