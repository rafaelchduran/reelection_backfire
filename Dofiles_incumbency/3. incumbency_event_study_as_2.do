...
eststo: xi: reghdfe  `j'  reform inc_party_won interaction_ref  pol`pol'   i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
		estadd local fixed \checkmark
		estadd local year \checkmark
		estadd local controls  
		estadd local polinomial linear  		
}
foreach pol in 2 {
rdbwselect  `j' mv_incparty, c(0) p(`pol') kernel(tri) bwselect(CCT) 
global optimal = e(h_CCT)
...
esttab using "../Tables/naive_twfe.tex", replace f b(%9.3f) se(%9.3f) se  star(* 0.10 ** 0.05 *** 0.01) ///
s(N r2 fixed year weights, fmt(0 3) ///
label("Observations" "R-squared"  "Country FE" "Year FE" "Weights$^a$")) ///
keep(ltariff_revenues) ///
coeflabel(ltariff_revenues "log(tariff revenues)") ///
mgroups("\begin{tabular}[c]{@{}l@{}} Polity IV's \\ Political Competition\end{tabular}" ///
 "\begin{tabular}[c]{@{}l@{}} Vanhanen's \\ Political Competition\end{tabular}"  ///
  "\begin{tabular}[c]{@{}l@{}} V-Dem's \\ Equal Competition \end{tabular}"  ///
 "\begin{tabular}[c]{@{}l@{}}  V-Dem's  \\ Equal Protection \end{tabular}" , ///
