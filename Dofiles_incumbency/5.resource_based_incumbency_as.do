...
*3) outcomes
	foreach i in ingresos2{
	replace `i'=`i'/1000000
	}	
	foreach i in t_person{
	replace `i'=(`i'/pop)*1000
	}
	global outcomes t_person ingresos2
	global outcome1 t_person
	global outcome2 ingresos2
...
bysort inegi: carryforward $outcome1, replace
...
ytitle(" ")  xtitle(" ") ///
subtitle("Panel D: Bureaucrats per 1,000 inh.") legend(order(1 "99% CI" 1 "95% CI" 3 "90% CI" ///
...
coefplot (est1, rename((1) = "t-6") msize(small) mcolor(red) levels(99 95 90) ciopts(lwidth(*0.5 *1 *3) color(black black black))) ///
 ...
 vertical scheme(s1color)  yline(0)    ///
ytitle(" ")  xtitle(" ") ///
subtitle("Panel F: Total Revenues (Mill. pesos)") legend(order(1 "99% CI" 1 "95% CI" 3 "90% CI" ///
4 "linear" 20 "quadratic" ) rows(2)) 
graph export "../Figures_incumbency/ingresos.png", as(png) replace
graph export "../Figures_incumbency/ingresos.pdf", as(pdf) replace
graph export "../Figures_incumbency/ingresos.tif", as(tif) replace
graph save "../Figures_incumbency/ingresos.gph", replace

****COMBINE
*combine:
grc1leg "../Figures_incumbency/human_resources.gph" "../Figures_incumbency/ingresos.gph" , ///
scheme(s1color)  imargin(vsmall)  col(2)  l1(" ") b1(" ")
graph export "../Figures_incumbency/resources_based_incumbency.png", as(png) replace
graph export "../Figures_incumbency/resources_based_incumbency.pdf", as(pdf) replace
graph export "../Figures_incumbency/resources_based_incumbency.tif", as(tif) replace
