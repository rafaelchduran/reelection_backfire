...
global quadraticpolynomial  reform_pol2 inc_party_won_pol2 interaction_ref_pol2
...
preserve
foreach n in 1 2{
foreach j in $outcome2 $outcome5 {
*subset sample:
foreach pol in 1 {
rdbwselect  $outcome5 mv_incparty, c(0) p(`pol') kernel(tri) bwselect(CCT) 
global optimal = e(h_CCT)/`n'

xi: reghdfe  $outcome5  reform inc_party_won interaction_ref  pol`pol' $linearpolynomial $controls_time_acuerdo  i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
*keep if e(sample)==1
}
...
eststo: xi: reghdfe  `j'  reform inc_party_won interaction_ref  pol`pol' $linearpolynomial $controls_time_acuerdo    i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
...
eststo: xi: reghdfe  `j'  reform inc_party_won interaction_ref  pol`pol' $quadraticpolynomial $controls_time_acuerdo  i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
		estadd local fixed \checkmark
		estadd local year \checkmark
		estadd local controls  
...
eststo: xi: reghdfe  `j' pol`pol' inc_party_won inc_party_won_pol2  reform reform_pol2  interaction_ref interaction_ref_pol2 $linearpolynomial  $controls_time_acuerdo    i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
		estadd local fixed \checkmark
		estadd local year \checkmark
		estadd local controls  
		estadd local polynomial quadratic
...
label variable interaction "Personal"
...
eststo: xi: reghdfe  `j'  $lagsleads inc_party_won interaction  pol`pol'   i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
}
}
...
(est2, msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 ...
 keep($lagsleads inc_party_won interaction)
ytitle(" ")  xtitle(" ") ///
subtitle("Panel A: Probability of winning at Election t+1") legend(order(1 "99% CI" 1 "95% CI" 3 "90% CI" ///
4 "linear" 20 "quadratic" ) rows(2)) 
graph export "../Figures_incumbency/new_naive_incumbency.png", as(png) replace
graph export "../Figures_incumbency/new_naive_incumbency.pdf", as(pdf) replace
graph export "../Figures_incumbency/new_naive_incumbency.tif", as(tif) replace
graph save "../Figures_incumbency/new_naive_incumbency.gph", replace
...
(est4, msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 ...
 keep($lagsleads inc_party_won interaction)
ytitle(" ")  xtitle(" ") ///
subtitle("Panel A: Vote Share at Election t+1") legend(order(1 "99% CI" 1 "95% CI" 3 "90% CI" ///
4 "linear" 20 "quadratic" ) rows(2)) 
graph export "../Figures_incumbency/new_naive_incumbency_margin.png", as(png) replace
graph export "../Figures_incumbency/new_naive_incumbency_margin.pdf", as(pdf) replace
graph export "../Figures_incumbency/new_naive_incumbency_margin.tif", as(tif) replace
graph save "../Figures_incumbency/new_naive_incumbency_margin.gph", replace
...
*global inter_pol2 lag_8_2018_pol2 lag_6_2018_pol2 lag_6_2016_pol2 lag_5_2018_pol2 lag_5_2015_pol2 lag_4_2018_pol2 lag_4_2017_pol2 lag_4_2015_pol2    date_0_2018_pol2 date_0_2017_pol2 date_0_2016_pol2 date_0_2015_pol2  
	...
	* to use: global controls_time_acuerdo  presidential_election pan_president $homicides $margin $governor $margin_governor $effectiveparties $logpop
	...
	global controls_time_acuerdo presidential_election     $governor  
...
*Generate interaction: 	
gen interaction=inc_party_won*date_0_2015
...
foreach i in lag_6 {
qui xi: reghdfe  `j'  $saturated inc_party_won interaction  pol`pol'    $controls_time_acuerdo  i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
...
qui xi: reghdfe  `j'  $saturated inc_party_won interaction  pol`pol'    $controls_time_acuerdo  i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
 ...
 xi: reghdfe  `j'  $saturated inc_party_won interaction  pol`pol'    $controls_time_acuerdo  i.year if mv_incparty<${optimal} & mv_incparty>-${optimal}, a(inegi) vce(cluster estado)
	eststo: lincomest  	_b[interaction]	
}
...
coefplot (est1, rename((1) = "t-6") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 ...
 (est4, rename((1) = "Term vs. non-term limited") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 ...
 (est7, rename((1) = "t-6") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est8, rename((1) = "t-5") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est9, rename((1) = "t-4") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est10, rename((1) = "Term vs. non-term limited") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
...
subtitle("Panel A: Probability of winning at Election t+1") legend(order(1 "99% CI" 1 "95% CI" 3 "90% CI" ///
4 "linear" 20 "quadratic" ) rows(2)) 
graph export "../Figures_incumbency/new_incumbency.png", as(png) replace
graph export "../Figures_incumbency/new_incumbency.pdf", as(pdf) replace
graph export "../Figures_incumbency/new_incumbency.tif", as(tif) replace
graph save "../Figures_incumbency/new_incumbency.gph", replace

*Figure: Vote share in t+1. Pol1 and Pol2			   
coefplot (est13, rename((1) = "t-6") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est14, rename((1) = "t-5") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est15, rename((1) = "t-4") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est16, rename((1) = "Term vs. non-term limited") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est17, rename((1) = "Partisan") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est18, rename((1) = "Personal") msize(large) mcolor(red) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est19, rename((1) = "t-6") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est20, rename((1) = "t-5") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 (est21, rename((1) = "t-4") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est22, rename((1) = "Term vs. non-term limited") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est23, rename((1) = "Partisan") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black)))  ///
 (est24, rename((1) = "Personal") msize(large) mcolor(blue) levels(99 95 90) ciopts(lwidth(*1 *3 *5) color(black black black))) ///
 , ///
 vertical scheme(s1color)  yline(0)    ///
ytitle(" ")  xtitle(" ") ///
subtitle("Panel B: Vote share at Election t+1") legend(order(1 "99% CI" 1 "95% CI" 3 "90% CI" ///
4 "linear" 20 "quadratic" ) rows(1)) 
graph export "../Figures_incumbency/new_incumbency_margin.png", as(png) replace
graph export "../Figures_incumbency/new_incumbency_margin.pdf", as(pdf) replace
graph export "../Figures_incumbency/new_incumbency_margin.tif", as(tif) replace
graph save "../Figures_incumbency/new_incumbency_margin.gph", replace

*=============================================
*Combine figures:
grc1leg "../Figures_incumbency/new_incumbency.gph" "../Figures_incumbency/new_incumbency_margin.gph" , ///
scheme(s1color)  imargin(vsmall) ycommon  col(1)
graph export "../Figures_incumbency/new_paralleltrend_incumbency.png", as(png) replace
graph export "../Figures_incumbency/new_paralleltrend_incumbency.pdf", as(pdf) replace
graph export "../Figures_incumbency/new_paralleltrend_incumbency.tif", as(tif) replace
